  ...
  const renderExpandedContent = (ord) => (
    <div className={styles.details}>
      ...
      <div className={styles.qaSection}>
        ...
        <p>
          11. Original Charger?{" "}
          <span className={styles.answer}>Yes</span>
        </p>
      </div>
        ...
        {orders?.map((ord) => (
          <div key={ord._id} className={styles.card}>
            ...
            <div className={styles.viewMoreContainer}>
              ...
              >
                {!isMobile && expandedOrderId === ord._id ? "Hide" : "View more"}
              </span>
            </div>

            {/* Expanded section for desktop */}
            {!isMobile && expandedOrderId === ord._id && renderExpandedContent(ord)}
          </div>
        ))}
      </div>
              ...
              <div className={styles.modalDeviceInfo}>
                ...
                <div>
                  <div className={styles.modalDeviceTitle}>
                    {currentOrder?.deviceEvaluationId?.deviceName || "Unknown Device"}
                    {currentOrder?.deviceEvaluationId?.deviceVariant
                      ? ` (${currentOrder.deviceEvaluationId.deviceVariant})`
                      : ""}
                  </div>
                  <div className={styles.modalDevicePrice}>
                    ₹{currentOrder?.deviceEvaluationId?.finalPrice?.toLocaleString() || "0.00"}
                  </div>
                  <div className={styles.modalOrderId}>Order ID: {currentOrder.orderId}</div>
                </div>
              </div>
