...
const Login = ({ onSwitchToSignup, onSuccessLogin, setShowLoginModal }) => {
  ...
  const { setUser,userSelection, isLoginModalOpen, setIsLoginModalOpen } =
  ...
  const handleSignUp = async (e) => {
    ...
    try {
       const cityId = userSelection?.cityId; // city id from context
      const { data } = await api.post("/sell-module/user/signUp", {
        phone: mobile,
        verifyOtp: otp.join(""),
          cityId, // <-- send selected city here
      });

      if (data?.token) {
        Cookies.set("accessToken", JSON.stringify(data?.token), {
          expires: 2,
          sameSite: "strict",
        });
        setUser(data?.user);
            ...
            {!otpSent ? (
              ...
              <>
                ...
                <div className={styles.otpActions}>
                  <span className={styles.link}>Get on call</span>
                  {timer > 0 ? (
                    <span className={styles.link}>
                      Resend OTP in {timer}s
                    </span>
                  ) : (
                    <span className={styles.link} onClick={handleSendOtp}>
                      Resend OTP
                    </span>
                  )}
                </div>
