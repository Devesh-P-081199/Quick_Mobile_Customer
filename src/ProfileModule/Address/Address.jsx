...
const Address = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(`Failed to ${isEditing ? 'update' : 'submit'} address`);
    }
  };
  ...
  const fetchZipDetails = async (zipcode) => {
    try {
      const response = await axios.get(`http://65.1.41.179:8080/api/sell-module/user/getZipDetails/${zipcode}`);
      ...
      console.error(error);
      toast.error('Failed to fetch city from pincode');
      setCityName('');
      setFormData((prev) => ({ ...prev, cityName: '' })); // <- Also reset in formData
    }
  };
          ...
          <form className={`${styles.addressWrapper} scrollbar-hidden`} onSubmit={handleSubmit}>
              ...
              {addressBoxes?.map((address, index) => (
                ...
                >
                  ...
                  <div className="absolute left-4 top-[45px] flex flex-col items-start p-0 gap-1 text-left">
                    ...
                    <p className="text-[14px] text-black">
                      {(address?.cityName || 'N/A')}, {(address.state || 'N/A')}
                    </p>
                    <p className="text-[14px] text-black">
                      Zip: {address.zipCode || 'N/A'}
                    </p>
                  </div>
                  <button className="absolute left-16 transform -translate-x-1/2 top-3 bg-black text-white text-xs font-medium px-2 py-1  rounded-md home-office-button">
            ...
            {(isAddressOpen || isEditing) && (
              <div className={styles.AddressBox}>
                ...
                <h3 className={styles.addressheading}>{isEditing ? 'Edit Address' : 'Add New Address'}</h3>
