...
const EditProfile = () => {
  ...
  const handleUpdateProfile = async () => {
    try{
      ...
      // update user context  
      setUser((prev) => ({ ...prev, name: ProfileUpdate.name, email: ProfileUpdate.email }));
      // update cookies 
      Cookies.set("user", JSON.stringify({ ...user, name: ProfileUpdate.name, email: ProfileUpdate.email }), {
        expires: 2,
        sameSite: "strict",
      });
