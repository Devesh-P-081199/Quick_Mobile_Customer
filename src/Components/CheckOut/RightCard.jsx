...
function RightCard() {
  const { selectedAddress,selectedPaymentMethod,currentEvaluationId} = useContext(UserContext);
 ...
 // console.log("Sleetced payment method",selectedPaymentMethod)
  ...
  useEffect(() => {
    console.log("Current Evaluation ID in RightCard:", currentEvaluationId);
     ...
     <div className={styles.RightBox}>
      <div className={styles.Frame1597882160}>
        <div className={styles.ImageContainer}>
          <img src={data?.devicePic || MobileIcon} alt="Apple iPhone 16 Pro Max" />
        </div>
        <div className={styles.Frame1597882163}>
          <span className={styles.ProductName}>
            {data?.deviceName} {data?.deviceVarian?`(${data?.deviceVariant})`:''}
          </span>
        </div>
      </div>
      ...
      <button onClick={handlePlaceOrder} className={styles.sellNow}>
        {location.pathname === `/${slug}/payment-mode-selection` ? "Place Order" : "Continue"}
      </button>
