...
const fetchSeriesModels = async () => {
  try {
    const resp = await api.get(`/sell-module/user/fetchSeriesModels?option=Sell&brandId=${brandId}&seriesId=${seriesId}`);
      ...
      <div className={styles.wrapper}>
        ...
        <ul className={styles.seriesList}>
          {series?.map((series, index) => (
            ...
            >
              {series.seriesName}
              {seriesId === series._id && <span className={styles.crossIcon}> ❌</span>}
            </li>
          ))}
        </ul>
      </div>
          ...
          {model?.map((model, index) => (
            ...
            >
              <div className={styles.imgIndividual}>
                <img src={model?.devicePic || model.icon} alt={model.name} title={model.name} />
              </div>
              {model?.deviceName}
            </NavLink>
          ))}
