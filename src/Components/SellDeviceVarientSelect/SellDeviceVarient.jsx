  ...
  fetchVariantsByProductId(slug2).then((data) => {
    ...
    if (data.selectedVariantId) {
      ...
      setUserSelection((prev) => ({
        ...
        variantId: data.selectedVariantId,
        variantSlug: variantsArr.find(v => v._id === data.selectedVariantId)?.slug || "",
      }));
...
if (variantsArr.length === 1) {
  console.log("Only one variant found, auto-navigating...", variantsArr[0]);
  const variant = variantsArr[0];
          ...
          <div className={styles.leftContent}>
            <div className={styles.leftImgBox}>
              {isImageLoading ? (
                ...
                <img src={variants.productId.devicePic} alt={variants.productId?.deviceName} title={variants.productId?.deviceName} />
              )}
              <button
