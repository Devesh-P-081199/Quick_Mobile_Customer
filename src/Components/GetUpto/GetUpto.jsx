...
const GetUpto = () => {
  const { userSelection, setPackages, deviceInfo, setDeviceInfo,setUserSelection,toggleModal } =
  ...
  const [seoData, setSeoData] = useState(null); // null means loading
  ...
  const FetchPriceAndPackages = async () => {


    //console.log("Fetching price and packages with userSelection:", userSelection);
    ...
    console.log("Final slugs being used in fertch:", slug1, slug2);
...
//   setUserSelection((prev) => ({
//     ...prev,
//     variantSlug: prev.variantSlug || slug2,
//     catSubcatSlug: prev.catSubcatSlug || slug1,
//   }));
// }
     ...
     //console.log("Updated userSelection:", userSelection);
        ...
        const response = await api.get(
          `/sell-module/user/packages-price/${slug2}/${userSelection.cityId}`
        );
        ...
        setPrices([12000, 17000]); // You can update these based on your logic or response
      } catch (error) {
  ...
  if (userSelection?.cityId && userSelection?.variantSlug) {
    FetchPriceAndPackages();
  } else if (!userSelection?.cityId) {
    toast.error("Please select city first");
    console.log("City ID missing in userSelection");
    toggleModal(true);
    console.log("Modal opened for city selection");
  }
}, [userSelection?.variantSlug, userSelection?.cityId]);
          ...
          <div className={styles.leftContent}>
            <div className={styles.leftImgBox}>
              {deviceInfo?.devicePic ? (
                <img src={deviceInfo.devicePic} alt={deviceInfo.deviceName} title={deviceInfo.deviceName}/>
              ) : (
          ...
          <div className={styles.productDetails}>
            ...
            <div className={styles.extraInfo}>
              ...
              <div className={styles.highlightBox}>
                Apple: Go to “Settings” {">"} “General” {">"} “iPhone Storage”.
                You will see a breakdown of your used and available storage,
                along with app-wise storage usage.
              </div>
            </div>
