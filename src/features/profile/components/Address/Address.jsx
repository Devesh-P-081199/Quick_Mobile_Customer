...
const Address = () => {
  ...
  const getSavedAddresses = async () => {
    try {
      ...
      setAddressBoxes(resp.data.data?.addresses || []);
    } catch (error) {
  ...
  const fetchZipDetails = async (zipcode) => {
    try {
      const response = await axios.get(
        `http://65.1.41.179:8080/api/sell-module/user/getZipDetails/${zipcode}`
      );
      ...
      <section>
        <div className={`${styles.addressContainer} scrollbar-hidden`}>
          <div className={styles.left}>
            <form
              className={`${styles.addressWrapper} scrollbar-hidden`}
